...
describe('getLogsAuditoria filtros', () => {
  ...
  it('aplica gte, lte, eq e ilike según parámetros', async () => {
    ...
    expect(builder.calls.some((c: string) => c.startsWith('gte:'))).toBe(true)
    expect(builder.calls.some((c: string) => c.startsWith('lte:'))).toBe(true)
    expect(builder.calls.some((c: string) => c === 'eq:action')).toBe(true)
  ...
  it('no aplica eq de acción si es todas', async () => {
    ...
    expect(builder.calls.filter((c: string) => c === 'eq:action').length).toBe(0)
  })
})
